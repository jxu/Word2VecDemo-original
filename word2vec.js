...
async function loadModel() {
    ...
    const {data} = await axios.get("https://www.cs.cmu.edu/~dst/Word2VecDemo/wordvecs77000.json");
...
async function newwords(op=0) {
    ...
    else if(text_vals.includes(word)){
        errornew.innerHTML = "The word already exists in the scatter plot.";
    }
    else{
...
async function delete_words(op=0) {
    let old_vec;
    var word = document.getElementById("del_Form").elements[0].value;
    if (wordvectors.vectors[word] == null){
        errornew.innerHTML = "The word is out of vocabulary.";
    }
    else if(!text_vals.includes(word)){
        errornew.innerHTML = "The word is not present in the scatter plot.";
    }
    else{
        document.getElementById("del_Form").elements[0].value = '';
        errornew.innerHTML = "";
        old_vec = tf.tensor1d(wordvectors.vectors[word]);
        old_vec = normalization(old_vec);

        let old_vec_x =  tf.sub(1 , tf.dot(old_vec,MEAN_no_gender_no_age)).dataSync()[0];
        let old_vec_y =  tf.dot(old_vec,MEAN_gender).dataSync()[0];
        let old_vec_z =  tf.dot(old_vec,MEAN_age).dataSync()[0];

        index_xvals =x_vals.indexOf(old_vec_x);
        index_yvals =y_vals.indexOf(old_vec_y);
        index_zvals =z_vals.indexOf(old_vec_z);
        index_text_vals =text_vals.indexOf(word);
        console.log(text_vals);
        x_vals.splice(index_xvals, 1);
        y_vals.splice(index_yvals, 1);
        z_vals.splice(index_zvals, 1);
        text_vals.splice(index_text_vals, 1);
        console.log(text_vals);
